  ...
  methods: {
    ...
    savePhoto () {
      ...
      fetch(this.imageData)
        .then(res => res.blob())
        .then((blob) => {
          this.uploadImageToStrapi(blob, `phototicket${this.ticket.id}`, 'image/jpeg')
        })
    },
    async uploadImageToStrapi (imageBlob, imageName, imageType) {
      const formData = new FormData()
      formData.append('files', imageBlob, `${imageName}.${imageBlob.type === 'image/jpeg' ? 'jpg' : 'png'}`)
      ...
      await fetch(`${this.$config.API_STRAPI_ENDPOINT}upload`, {
        ...
        if (input.status === 200) {
          ...
          throw new Error('Upload failed', input)
        }
      }).catch((error) => {
