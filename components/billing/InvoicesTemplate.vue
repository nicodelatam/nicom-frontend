<template>
  <v-card v-if="invoices.length > 0 && service && company" class="elevation-0 bill-card" light>
    <div class="header-section">
      ...
      <div class="receipt-info">
        <div class="receipt-box">
          <h5 class="receipt-title">FACTURAS PENDIENTES</h5>
          <h5 class="receipt-number">{{ service.id }}</h5>
          <h5 class="receipt-code">CÓDIGO: {{ service.code }}</h5>
        </div>
      </div>
    </div>
    ...
    <div class="user-data-grid">
      <div class="user-data-section">
        ...
        <div class="data-value">{{ service.client_name }}</div>
      </div>

      <div class="user-data-section">
        <div class="data-label">NO. DOCUMENTO</div>
        <div class="data-value">{{ service.dni }}</div>
      </div>

      <div class="user-data-section">
        <div class="data-label">SERVICIO</div>
        <div class="data-value">{{ service.offer.name }}</div>
      </div>

      <div class="user-data-section">
        <div class="data-label">DIRECCIÓN DE DOMICILIO</div>
        <div class="data-value">{{ service.address }}</div>
      </div>

      <div class="user-data-section">
        <div class="data-label">BARRIO</div>
        <div class="data-value">{{ service.neighborhood }}</div>
      </div>

      <div class="user-data-section">
        <div class="data-label">ID DEL USUARIO</div>
        <div class="data-value">CÓDIGO: {{ service.code }}</div>
      </div>

      <div class="user-data-section">
        <div class="data-label">CELULAR</div>
        <div class="data-value">{{ service.phone }}</div>
      </div>

      <div class="user-data-section">
        <div class="data-label">CORREO ELECTRÓNICO</div>
        <div class="data-value">{{ service.normalized_client.email }}</div>
      </div>
    ...
    <div class="footer-section">
      <div class="payment-status-container">
        <div>
          <span class="status-advance">PENDIENTE</span>
        </div>
        <div class="balance-info">
          <strong>TOTAL PENDIENTE POR PAGAR: ${{ Number(invoices.reduce((total, item) => {
            if (item.payed) {
              return total
            } else {
              return total + item.balance
            }
          }, 0)).toLocaleString('es') }} pesos</strong>
        </div>
      </div>
